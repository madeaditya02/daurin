...
<template>
  <div
        class="bg-gray-100 px-6 sm:px-10 md:px-20 lg:px-32 pt-5 md:pt-12 border-y"
      >
    ...
    <div class="mt-8 flex gap-4 md:gap-10 overflow-x-auto">
      <a
        href=""
        class="pb-3 font-medium border-b-2 border-blue-500 text-blue-500"
        >Semua</a
      >
  ...
  <div class="px-6 sm:px-10 md:px-20 lg:px-32 py-5 md:py-8">
    <div
      class="rounded-xl border bg-gray-50 px-5 md:px-8 py-3 md:py-5 mt-6 first:mt-0"
    >
      <div class="flex justify-between items-center flex-wrap gap-y-4">
        <div>
          <div class="flex gap-3 md:gap-6 items-center">
            ...
            <span
              class="px-2.5 py-1 rounded font-bold text-sm bg-yellow-100 text-yellow-800"
              >Pending</span
            >
          </div>
          <div
            class="flex gap-x-10 gap-y-3 flex-wrap flex-col sm:flex-row mt-3 sm:mt-4 text-sm sm:text-base"
          >
      ...
      <div class="mt-4 flex gap-3 text-xs">
        <span class="px-3 py-1.5 rounded-full bg-primary/60 text-white"
          >Botol Plastik</span
        >
        <span class="px-3 py-1.5 rounded-full bg-primary/60 text-white"
          >Kaleng Bekas</span
      ...
      <button
        class="mt-4 px-3 py-1.5 text-sm sm:px-4 sm:py-2 sm:text-base rounded-md bg-primary text-white" @click="openDetailBarang"
      >
        Detail Barang
      </button>
    </div>
  ...
  <UModal v-model="showDetailBarang">
    <div class="px-6 py-6 rounded-2xl">
      ...
      <div class="mt-3 px-3 py-2 rounded-lg border flex justify-between items-center" v-for="item in detailBarang">
        <div>
          <h3 class="text-lg"><span class="font-semibold">{{ item.item }}</span> ({{ item.amount }} {{item.itemUnit}})</h3>
